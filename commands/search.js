  ...
  async run(message, args, client, slash) {
    ...
    for (let i = 0; i < searchResult.tracks.length; i++) {
      ...
      embeds.push({
        ...
        description: `**${i + 1}.** [${searchResult.tracks[i].title}](${
          searchResult.tracks[i].url
        })\n${searchResult.tracks[i].author} - ${
          searchResult.tracks[i].duration
        }`,
        color: 0x44b868,
      });
    }
    ...
    const msg = slash
      ? await message.editReply({ embeds: embeds, components: [row] })
      : await message.reply({
          embeds: embeds,
          components: [row],
          failIfNotExists: false,
    ...
    const collector = await sMessage.createMessageComponentCollector({
      filter,
      time: 30000,
    });
    ...
    collector.on("collect", async (button) => {
      ...
      queue.addTrack(
        searchResult.tracks[parseInt(button.customId.split("_").pop())]
      );
      sMessage.edit({
        embeds: [
          {
